...
def featurize(sentence: str, embeddings: gensim.models.keyedvectors.KeyedVectors) -> Union[None, torch.FloatTensor]:
    ...
    # if no tokens matched the embedding vocabulary -> return None
    # i.e the sentence is empty or None of the words in the sentence is in the embedding vocabulary
    if len(vectors) == 0:
        return None

    # average the word vectors -> numpy array of shape (embed_dim,)
    avg = np.mean(np.vstack(vectors), axis=0)

    # convert numpy to torch tensor (FloatTensor)
...
class SentimentClassifier(nn.Module):
    def __init__(self, embed_dim, num_classes):
        ...
        # linear classifier - input: average embedding vector of each sequence, output: 2 logits
...
def accuracy(logits: torch.FloatTensor, labels: torch.LongTensor) -> torch.FloatTensor:
    ...
    is_correct = (predicted_labels == labels).to(torch.float32)
    return is_correct
